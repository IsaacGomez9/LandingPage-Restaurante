...
const Ramen = async (Platos) => {

    let inyectar = document.querySelector('#platosJunt')
    inyectar.innerHTML = ""
    const Ramen = Platos.filter(category => category.categories == 'Ramen')
    Ramen.map(plato => {
        const { categories, description } = plato
        ...
        <div>${categories}</div>
          <div>
            <img src="./Kebab.png " class="" style="width: 180px" />
          </div>
